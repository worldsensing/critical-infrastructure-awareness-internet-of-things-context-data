...
@pytest.mark.parametrize("name",
                         ["Thing1"])
def test_get_thing_not_exist(client: TestClient,
                             name: str):
    response = client.get(f"{prefix}/{name}/")
...
@pytest.mark.parametrize("name, sensor_1_name, sensor_2_name, location_name",
                         [["Thing1", "Sensor1", "Sensor2", "Location1"],
                          ["Thing2", None, None, None]])
def test_create_thing(client: TestClient, session: Session,
    ...
    response = client.post(f"{prefix}/", json=thing_json)
    assert response.status_code == 200
    assert response.json()["id"] == 1
    assert response.json()["name"] == name
    assert response.json()["location_name"] == location_name
...
@pytest.mark.parametrize("name, location_name",
                         [["Thing1", "Location1"],
                          ["Thing2", None]])
def test_delete_thing_exist(client: TestClient, session: Session,
                            name: str, location_name: str):
    # PRE
    create_thing(session, name=name, location_name=location_name)
    # TEST
...
@pytest.mark.parametrize("name",
                         ["Thing1"])
def test_delete_thing_not_exist(client: TestClient, session: Session,
                                name: str):
    response = client.delete(f"{prefix}/{name}/")
    assert response.status_code == 404
    assert response.json() == {"detail": "Thing not found"}
