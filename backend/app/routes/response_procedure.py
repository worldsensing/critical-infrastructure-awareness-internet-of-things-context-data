...
def post_response_procedure(response_procedure: ResponseProcedure,
    ...
    if response_procedure.actuators:
        for actuator in response_procedure.actuators:
            db_actuator = actuator_repo.get_actuator(actuator_name=actuator.name,
                                                     session=session)
            if not db_actuator:
...
@router.get("/{response_procedure_name}/actuators", response_model=List[Actuator])
def get_response_procedure_actuators(response_procedure_name: str,
                                     session: Session = Depends(get_session)):
    db_response_procedure = get_response_procedure(
        response_procedure_name=response_procedure_name, session=session)

    return db_response_procedure.actuators
...
def delete_response_procedure(response_procedure_name: str,
                              session: Session = Depends(get_session)):
    get_response_procedure(response_procedure_name=response_procedure_name, session=session)
