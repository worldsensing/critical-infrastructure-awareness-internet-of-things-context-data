...
def get_event_rules_from_event_rule_name(event_rule_name: str):
    url = f"{BASE_URL}/{API_CONTEXT_AWARE_RULES_EVENT_RULES_ENDPOINT}/{event_rule_name}/"
    print(f"Send GET to obtain EventRule from EventRule name {event_rule_name}: {url}")

    r = requests.get(url)
    event_rule = json.loads(r.content)
